...
var Name__namespace = /*#__PURE__*/_interopNamespace(Name);
...
function exportPublicKey(key) {
    return (key instanceof CryptoKey
        ...
        return "-----BEGIN PUBLIC KEY-----\n".concat(ethers.encodeBase64(new Uint8Array(exported)), "\n-----END PUBLIC KEY-----");
    });
}
...
function importPrivateKey(input) {
    ...
    return subtle$1.importKey("pkcs8", ethers.decodeBase64(pemContents), {
        name: "ECDH",
        namedCurve: "P-521",
...
function importPublicKey(input) {
    ...
    var pemContents = input.substring(pemHeader.length, input.length - pemFooter.length);
    return subtle$1.importKey("spki", ethers.decodeBase64(pemContents), {
        name: "ECDH",
        namedCurve: "P-521",
    }, true, []);
}
var getNameFromSeed = function (seed) {
    return libp2pCrypto.keys
        .generateKeyPairFromSeed("Ed25519", seed, 2048)
        .then(function (keys) { return new Name__namespace.WritableName(keys); });
};
var getNameAsBytes = function (w3name) {
    return base36.base36.baseDecode(w3name.toString());
};
var parseNameFromBytes = function (nameBytes) {
    return base36.base36.baseEncode(nameBytes);
};
var bytesToName = function (nameBytes) {
    return Name__namespace.parse(base36.base36.baseEncode(nameBytes));
};
var loadName = function (w3name) {
    return Name__namespace.resolve(w3name)
...
var readValueAt = function (w3name) {
    return loadName(w3name).then(function (revision) { return (revision === null || revision === void 0 ? void 0 : revision.value) && ethers.decodeBase64(revision.value); });
};
var readValueAtAddress = function (address) {
    return readValueAt(Name__namespace.parse(address));
};
...
var _writeAt = function (message, slot, increment) {
    if (increment === void 0) { increment = false; }
    var commit = function (revision) { return Name__namespace.publish(revision, slot.key); };
    var encodedMessage = ethers.encodeBase64(message);
    if (!increment)
        return Name__namespace.v0(slot, encodedMessage).then(commit);
    return loadName(slot)
        .then(function (revision) {
        return revision
            ? Name__namespace.increment(revision, encodedMessage)
            : Name__namespace.v0(slot, encodedMessage);
    })
        .then(commit);
};
    ...
    return function (message, slot, increment) {
        ...
        return subtle$1
            ...
            var _message = concatBytes([
                ...
                ethers.getBytes(ethers.zeroPadValue(ethers.toBeArray(getTimestamp()), 8)),
                message,
            ]);
            return Promise.all([
            ...
            return writeAt(concatBytes([ethers.getBytes(signature), _message]), slot, increment);
        });
    };
};
var getSlotAt = function (currentSlot, i) {
    if (i === 0)
        return ethers.getBytes(currentSlot);
    return getSlotAt(ethers.sha256(ethers.hexlify(currentSlot)), i - 1);
};
...
var isEmpty = function (data) {
    return ethers.dataLength(ethers.stripZerosLeft(data)) === 0;
};
...
function calculateRepeatingByteLength(schema, data) {
    ...
    for (var key in schema) {
        var s = schema[key];
        if (s.type === "repeating") {
            var n = ethers.toNumber(data.slice(dataLength, dataLength + 4));
            dataLength += 4;
            for (var i = 0; i < n; ++i) {
...
function applySchema(schema, data) {
    ...
    for (var key in schema) {
        ...
        if (s.type === "boolean") {
            ...
            result[key] = ethers.toNumber(new Uint8Array(value));
        }
        else if (s.type === "buffer") {
            result[key] = new Uint8Array(value);
        }
        else if (s.type === "hexstring") {
            result[key] = ethers.hexlify(new Uint8Array(value));
        }
        else if (s.type === "bignumber") {
            result[key] = BigInt(ethers.hexlify(new Uint8Array(value)));
        }
            ...
            if (!s.schema)
                throw new Error("no schema specified for ".concat(key));
            var n = ethers.toNumber(value.slice(0, 4));
            needle += 4;
...
function buildSchema(schema, _data) {
    ...
    var _loop_1 = function (key) {
        ...
        if (s.type === "boolean") {
            ...
            result = ethers.getBytes(ethers.concat([result, ethers.zeroPadValue(bytes, (_a = s.byteLength) !== null && _a !== void 0 ? _a : bytes.length)]));
        }
        else if (s.type === "number" && s.byteLength) {
            if (typeof d !== "number") {
            ...
            result = ethers.getBytes(ethers.concat([result, ethers.zeroPadValue(ethers.toBeArray(d), s.byteLength)]));
        }
            ...
            result = concatBytes([
                result,
                ethers.getBytes(ethers.zeroPadValue(ethers.toBeArray(d.length), 4)),
...
function parseRequest(input) {
    try {
        var req = applySchema(requestSchema, ethers.decodeBase64(input));
        if (req.from !==
            ethers.verifyMessage(ethers.getBytes(ethers.concat([
                req.from,
                req.publicKey,
                ethers.zeroPadValue(ethers.toBeArray(req.publishedAt), 8),
                req.message,
...
var readWith = function (seed, keyholder) {
    return getNameFromSeed(seed)
        ...
        var _a = applySchema(requestSchema, ethers.decodeBase64(revision.value)); _a.from; _a.signature; var message = _a.message, publicKey = _a.publicKey; _a.publishedAt;
        /*
        if (
...
var KeyHolder = /** @class */ (function () {
    function KeyHolder(privateKey, signingKey) {
        ...
        this.wallet = ethers.isHexString(privateKey)
            ? new ethers.Wallet(privateKey)
            : privateKey.length > 100
                ? this.importIdentity(privateKey)
                : ethers.HDNodeWallet.fromMnemonic(ethers.Mnemonic.fromPhrase(privateKey));
    }
    ...
    KeyHolder.prototype.getContractRunner = function (address, abi, provider) {
        return new ethers.Contract(address, abi, new ethers.Wallet(this.wallet.privateKey, provider));
    };
    ...
    KeyHolder.prototype.encryptAndWrite = function (message, slot, publicKey, increment) {
        ...
        return (publicKey
            ...
            : importEncryptionKey(ethers.getBytes(this.wallet.privateKey)))
            .then(function (encryptionKey) {
    ...
    KeyHolder.prototype.buildSchemaAndSign = function (schema, data) {
        ...
        return this.wallet
            ...
            return concatBytes([ethers.getBytes(signature), message]);
        });
    };
    ...
    KeyHolder.prototype.receive = function (cipherText, slot, publicKey) {
        return (publicKey
            ? exchangeKeys(publicKey, this.signingKey.privateKey)
            : importEncryptionKey(ethers.getBytes(this.wallet.privateKey))).then(function (encryptionKey) {
    ...
    KeyHolder.prototype.createSecret = function (data) {
        return exportPrivateKey(this.signingKey.privateKey).then(function (privateKey) {
            return ethers.getBytes(ethers.sha256(ethers.concat([data, new TextEncoder().encode(privateKey)])));
        });
    };
    KeyHolder.prototype.createCommonSecret = function (data, publicKey) {
        return deriveBits(publicKey, this.signingKey.privateKey).then(function (commonBits) { return ethers.sha256(ethers.concat([data, commonBits])); });
    };
    KeyHolder.prototype.importIdentity = function (identity) {
        var identityBytes = ethers.decodeBase64(identity);
        var dc = new TextDecoder();
        this._signingKey = {
            privateKey: dc.decode(identityBytes.slice(32, 410)),
            publicKey: dc.decode(identityBytes.slice(410)),
        };
        return new ethers.Wallet(ethers.hexlify(identityBytes.slice(0, 32)));
    };
    KeyHolder.prototype.exportIdentity = function () {
        ...
        return Promise.all([
            ...
            var ec = new TextEncoder();
            return ethers.encodeBase64(ethers.concat([
...
var Manager = /** @class */ (function (_super) {
    ...
    function Manager(privateKey, signingKey) {
        ...
        _this.publishSlot = function (encodedSlotContent) {
            ...
            return Name__namespace.resolve(_this.rootName)
                .then(function (revision) { return Name__namespace.increment(revision, encodedSlotContent); })
                .catch(function (e) {
                if (e.message.startsWith("record not found")) {
                    return Name__namespace.v0(_this.rootName, encodedSlotContent);
                }
                throw e;
            })
                .then(function (revision) { return Name__namespace.publish(revision, _this.rootName.key); })
        ...
        _this.root = ethers.getBytes(ethers.sha256(new TextEncoder().encode(privateKey)));
        return _this;
    }
    ...
    Object.defineProperty(Manager.prototype, "nextSlot", {
        get: function () {
            this._slotRoot = ethers.getBytes(ethers.sha256(ethers.hexlify(this.slotRoot)));
    ...
    Manager.prototype.generateSlotRoot = function () {
        ...
        this._slotRoot = ethers.randomBytes(32);
    };
    Manager.prototype.generateSlotContent = function () {
        ...
        return Promise.all([
            ...
            return Promise.all([
                ...
                return ethers.encodeBase64(ethers.concat([signature, slotContent]));
            });
        });
    };
...
var readRoot = function (applicationRoot) {
    return loadName(Name__namespace.parse(applicationRoot)).then(function (revision) {
        if (revision === null)
            throw new Error("application root not found");
        return applySchema(signedSlotSchema, ethers.decodeBase64(revision.value));
    });
};
var readLastValue = function (applicationRoot, keyholder) {
    return loadName(Name__namespace.parse(applicationRoot)).then(function (revision) {
        if (revision === null)
            throw new Error("application root not found");
        var _a = applySchema(signedSlotSchema, ethers.decodeBase64(revision.value)); _a.signature; var slotRoot = _a.slotRoot; _a.publicKey;
        return findSlot(slotRoot)
            ...
            if (revision === null)
                return null;
            var _a = applySchema(requestSchema, ethers.decodeBase64(revision.value)), signature = _a.signature, from = _a.from, message = _a.message, publicKey = _a.publicKey;
            return {
                from: from,
                signature: signature,
                message: message,
...
var sendRequest = function (applicationRoot, keyholder, message, expectResponse, identity) {
    ...
    var responseSlot = ethers.randomBytes(32);
    return loadName(Name__namespace.parse(applicationRoot)).then(function (revision) {
        if (revision === null)
            throw new Error("application root not found");
        var _a = applySchema(signedSlotSchema, ethers.decodeBase64(revision.value)), signature = _a.signature, slotRoot = _a.slotRoot, publicKey = _a.publicKey;
        if (identity &&
            ethers.verifyMessage(concatBytes([slotRoot, publicKey]), signature) !== identity)
            throw new Error("malformed application root value");
        return findSlot(slotRoot)
            .then(function (_a) {
            _a[0]; var slot = _a[1];
...
exports.ByteLengthViolation = ByteLengthViolation;
exports.EventEmitter = EventEmitter;
exports.KeyHolder = KeyHolder;
exports.Service = Manager;
exports._writeAt = _writeAt;
exports.applySchema = applySchema;
exports.buildSchema = buildSchema;
exports.bytesToName = bytesToName;
...
exports.exchangeKeysAndDecrypt = exchangeKeysAndDecrypt;
exports.exchangeKeysAndEncrypt = exchangeKeysAndEncrypt;
exports.exportPrivateKey = exportPrivateKey;
exports.exportPublicKey = exportPublicKey;
exports.findSlot = findSlot;
exports.generateKeyPair = generateKeyPair;
exports.getNameAsBytes = getNameAsBytes;
exports.getNameFromSeed = getNameFromSeed;
exports.getSlotAt = getSlotAt;
...
exports.parseNameFromBytes = parseNameFromBytes;
exports.publishAt = publishAt;
exports.readLastValue = readLastValue;
exports.readRoot = readRoot;
exports.readSeedValue = readSeedValue;
exports.readValueAt = readValueAt;
exports.readValueAtAddress = readValueAtAddress;
